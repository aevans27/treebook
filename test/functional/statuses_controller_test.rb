...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should get index" do
    ...
    users(:drew).statuses.create(content: 'Non-blocked status')
    get :index
    assert_match /Non\-blocked status/, response.body
    assert_match /Blocked\ status/, response.body
  end

  test "should not display blocked user's posts when loged in" do
    sign_in users(:allan)
    users(:blocked_friend).statuses.create(content: 'Blocked status')
    users(:drew).statuses.create(content: 'Non-blocked status')
    get :index
  ...
  test "should create status when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:drew).id }
    end

    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:allan).id
  end
  ...
  test "should update status for current user when logged in" do
    sign_in users(:allan)
    put :update, id: @status, status: { content: @status.content, user_id: users(:drew).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:allan).id
  end
