...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:allan), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:allan), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
        @friendship3 = create(:requested_user_friendship, user: users(:allan), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
        @friendship4 = user_friendships(:blocked_by_allan)
  ...
  context "#new" do
    ...
    context "when logged in" do
      ...
      should "assign a new user friendship to the correct friend" do
        get :new, friend_id: users(:drew)
        assert_equal users(:drew), assigns(:user_friendship).friend
      end

      should "assign a new user friendship to the currently logged in user" do
        get :new, friend_id: users(:drew)
        assert_equal users(:allan), assigns(:user_friendship).user
      end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:drew)
        assert_match /Do you really want to friend #{users(:drew).full_name}?/, response.body
      end
    end
  end
    ...
    context "when logged in" do
      ...
      context "with a valid friend_id" do
        ...
        should "create a friendship" do
          assert users(:allan).pending_friends.include?(users(:mike))
        end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:allan), friend: @friend)
        create(:pending_user_friendship, friend: users(:allan), user: @friend)
        sign_in users(:allan)
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:allan))
        sign_in users(:allan)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        ...
        create(:accepted_user_friendship, friend: users(:allan), user: @friend)
