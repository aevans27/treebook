...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship works without raising exception" do
  	...
  	UserFriendship.create user: users(:allan), friend: users(:mike)
  end
end
...
context "a new instance" do
  setup do
    @user_friendship = UserFriendship.new user: users(:allan), friend: users(:mike)
  end
...
context "#send_request_email" do
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:allan), users(:drew)
      @friendship1 = users(:allan).user_friendships.where(friend_id: users(:drew).id).first
      @friendship2 = users(:drew).user_friendships.where(friend_id: users(:allan).id).first
end
...
should "accept the mutual friendship" do
  ...
  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:allan), users(:mike)
    end
    ...
    context ".request" do
      should "create two user friendships" do
        assert_difference 'UserFriendship.count', 2 do
          UserFriendship.request(users(:allan), users(:mike))
        end
      end
...
context "#block!" do
  setup do
    @user_friendship = UserFriendship.request users(:allan), users(:drew)
  end
  ...
  should "not allow new requests once blocked" do
    @user_friendship.block!
    uf = UserFriendship.request users(:allan), users(:drew)
    assert !uf.save
  end
end
end
